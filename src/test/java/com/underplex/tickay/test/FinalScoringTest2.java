...
public class FinalScoringTest2 {

	@Test
	public void test1() {

		System.out.println( "*************************************");
		System.out.println( "begin FinalScoringTest2" );
		System.out.println( "*************************************");
		...
		apu.getRoutes().addRoute( Finder.find( routes, "cadiz","lisboa") );
		apu.getPoints().addPoints( 2 );

		apu.getRoutes().addRoute( Finder.find( routes, "pamplona","madrid","white") );
		apu.getPoints().addPoints( 4 );

		barney.getRoutes().addRoute( Finder.find( routes, "stockholm","petro") );
		barney.getPoints().addPoints( 21 );
		
		carl.getRoutes().addRoute( Finder.find( routes, "pamplona","brest") );
		carl.getPoints().addPoints( 7 );
		
		carl.getRoutes().addRoute( Finder.find( routes, "pamplona","madrid","black") );
		carl.getPoints().addPoints( 4 );
		...
		Assert.assertEquals( 1, scores.get(0).getLongestTrack().size()); // has path of length 8, so one of the longest paths
		Assert.assertEquals( 1, scores.get(1).getLongestTrack().size()); // has path of length 8, so one of the longest paths
		Assert.assertEquals( 0, scores.get(2).getLongestTrack().size()); // has path of only 7
		Assert.assertEquals( 0, scores.get(3).getLongestTrack().size()); // has no routes, so no longest path
